...
import { Minus, Check, Pencil, X } from 'lucide-react';
		...
		yanking it out to align with figma/}
              {/* Section: Risk*/}
                    ...
                    <tr>
                        ...
                        <td id="dynamic-medical-option-data-holder"><Cell value={editable ? riskPrincipal : fmtCurrency(riskPrincipal) === '-' ? <Minus className="icon-minus" /> : fmtCurrency(riskPrincipal) } field="monthlyRiskContributionPrincipal" optionId={oid} editable={editable} type="number"/></td>
                        <td id="dynamic-medical-option-data-holder"><Cell value={editable ? riskAdult : fmtCurrency(riskAdult)  === '-' ? <Minus className="icon-minus" /> : fmtCurrency(riskAdult) } field="monthlyRiskContributionAdult" optionId={oid} editable={editable} type="number"/></td>
                        <td id="dynamic-medical-option-data-holder"><Cell value={editable ? riskChild : fmtCurrency(riskChild)  === '-' ? <Minus className="icon-minus" /> : fmtCurrency(riskChild)} field="monthlyRiskContributionChild" optionId={oid} editable={editable} type="number"/></td>
                        <td id="dynamic-medical-option-data-holder"><Cell value={editable ? riskChild2 : fmtCurrency(riskChild2) === '-' ? <Minus className="icon-minus" /> : fmtCurrency(riskPrincipal)} field="monthlyRiskContributionChild2" optionId={oid} editable={editable} type="number"/></td>
                    </tr>
                    {/* Section: MSA*/}
                    <tr className="modal-view-section-row">
                        <td colSpan='5' className="modal-view-section-cell">
                    ...
                    <tr>
                        ...
                        <td id="dynamic-medical-option-data-holder"><Cell value={editable ? msaPrincipal : fmtCurrency(msaPrincipal) === '-' ? <Minus className="icon-minus" /> : fmtCurrency(msaPrincipal)} field="monthlyMsaContributionPrincipal" optionId={oid} editable={editable} type="number"/></td>
                        <td id="dynamic-medical-option-data-holder"><Cell value={editable ? msaAdult : fmtCurrency(msaAdult) === '-' ? <Minus className="icon-minus" /> : fmtCurrency(msaAdult)} field="monthlyMsaContributionAdult" optionId={oid} editable={editable} type="number"/></td>
                        <td id="dynamic-medical-option-data-holder"><Cell value={editable ? msaChild : fmtCurrency(msaChild) === '-' ? <Minus className="icon-minus" /> : fmtCurrency(msaChild)} field="monthlyMsaContributionChild" optionId={oid} editable={editable} type="number"/></td>
                        <td id="dynamic-medical-option-data-holder"><Cell value={<Minus className="icon-minus" />} field="" optionId={oid} editable={false} type="number"/></td>
                    </tr>
                    </tbody>

                </table>
                {/* Section: Totals*/}
                {/* break it off into a different div -> Using a grid, maybe a dynamic grid */}
                {/*<tr className="modal-view-section-row">
              ...
              <tr className="modal-view-totals-row">
                ...
                <div className="section-divider">
                    <p id="monthly-contrib-breakdown">TOTAL MONTHLY DEDUCTION</p>
                    <Divider />

                </div>
                {/* Container wrapper with the necessary styling */}
                <div className="view-summary-container-wrapper">
                    {/* Div containing the total sums */}
                    <div className="view-summary-container">
                        <div className="view-summary-container-card-content">
                            <p id="header">PRINCIPAL</p>
                            <p id="value">{totalPrincipal ?? totalAdult}</p>
                        </div>
                        <div className="view-summary-container-card-content">
                            <p id="header">ADULT</p>
                            <p id="value">{totalAdult}</p>
                        </div>
                        <div className="view-summary-container-card-content">
                            <p id="header">1st CHILD</p>
                            <p id="value">{totalChild}</p>
                        </div>
                        <div className="view-summary-container-card-content">
                            <p id="header">2nd CHILD +</p>
                            <p id="value">{(totalChild2 === null && totalChild2 === 0 && totalChild === 0) ?  `Free` : totalChild}</p>
                        </div>
                    </div>
                </div>
            </>
        ...
        <div className="medicalaid-menu-background" onClick={onClose}>
            <div className="model-wrapper" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="medicalAidOptions-modalFooter">
                    <div className="section-divider" id="closing-section">
                        <Divider />
                    </div>

                    <button className="modal-footer-cancel-action-button" onClick={onClose}>
                        Cancel
                    </button>

                    <button
                        className="modal-footer-edit-action-button"
                        onClick={handleToggleEdit}
                    >
                        { isEditing ?
                            <>
                                <div className="save-edits-icon">
                                    <Check/>
                                </div>
                                <div className="save-edits-button-text-container">
                                    Save Plan
                                </div>
                            </>
                            :
                            <>
                                <div className="start-edits-icon">
                                    <Pencil />
                                </div>
                                <div className="start-editing-button-text-container">
                                    Edit Plan
                                </div>
                            </>
                        }
