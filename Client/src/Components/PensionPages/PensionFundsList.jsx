...
export default function PensionFundsList({isOpen, onClose}) {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      // ONLY ALLOW ONE ACTIVE FUND
      if (funds.length > 0 && !editingFund) {

        setFormError(
          "An active pension fund already exists. Delete it before creating another."
        );
          ...
          funds.map((f) => (
            ...
            <tr key={f.pensionFundId}>
              ...
              <td>
                ...
                {/* DELETE */}
                <span
                  className="clickable-tab delete-tab"
                  onClick={async () => {
                    ...
                    try {
                      ...
                      await deleteAllPensionOptions(
                        f.pensionFundId
                      );

                      await deletePensionFund(
                        f.pensionFundId
                      );
      ...
      {inactiveFunds.length === 0 ? (
        ...
        inactiveFunds.map((f) => (
          <tr key={f.pensionFundId}>
            ...
            <td>{f.description}</td>
            <td>{f.taxCode}</td>
            <td className="inactive-status">In‑active</td>
          </tr>
        ))
      )}
    </tbody>
