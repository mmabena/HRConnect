namespace HRConnect.Api.Controllers
{
    ...
    public class UserController : ControllerBase
    {
        ...
        [HttpPut("{UserId}")]
        public async Task<IActionResult> UpdateUser(int UserId, [FromBody] HRConnect.Api.DTOs.User.UpdateUserRequestDto updatedUser)
        {
            try
            {
        ...
        public async Task<IActionResult> CreateUser([FromBody] CreateUserRequestDto userDto)
        {
            try
            {
                ...
                return CreatedAtAction(nameof(GetUserById), new { UserId = created.UserId }, created.ToUserDto());
            }
            catch (ArgumentException ex)
            {
                ModelState.AddModelError("Validation", ex.Message);
