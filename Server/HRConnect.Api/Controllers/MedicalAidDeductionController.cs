...
public class MedicalAidDeductionController : ControllerBase
{
  ...
  [HttpPost("onboarding/eligible-options")]
  [ProducesResponseType(StatusCodes.Status200OK)]
  public async Task<ActionResult<IReadOnlyList<ResponseEligibileOptionsDto>>> GetEligibleOptionsForOnboarding(
  [FromBody] OnboardingMedicalEligibilityRequestDto request)
  {
    var result = await _eligibilityService.GetEligibleMedicalOptionsForOnboardingAsync(
    request.Salary,
    request.EmploymentStatus,
    request.EmployeeName,
    request.EmployeeSurname,
    new RequestEligibileOptionsDto
    {
