    ...
    public class OccupationalLevelService : IOccupationalLevelService
    {
            ...
            UpdateOccupationalLevelDto updateOccupationalLevelDto)
        {
            ...
            if (!string.IsNullOrWhiteSpace(updateOccupationalLevelDto.Description))
            {
                ...
                var duplicate = await _occupationalLevelRepo.GetOccupationalLevelByDescriptionAsync(trimmedDescription);

                if (duplicate != null && duplicate.OccupationalLevelId != id)
                    throw new DomainException("An occupational level with this description already exists.");
            ...
            {
                occupationalLevel.IsActive = updateOccupationalLevelDto.IsActive.Value;
            }
