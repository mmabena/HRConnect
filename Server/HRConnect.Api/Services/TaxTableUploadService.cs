namespace HRConnect.Api.Services
{
  ...
  public class TaxTableUploadService : ITaxTableUploadService
  {
    ...
    public async Task UploadTaxTableAsync(int taxYear, IFormFile file)
    {
      ...
      var existsForSameYear = await _context.TaxTableUploads
          .AnyAsync(x => x.TaxYear == taxYear && x.IsActive);

      if (existsForSameYear)
        throw new ArgumentException($"A tax table for the year {taxYear} already exists");

      var olderActiveUploads = await _context.TaxTableUploads
          .Where(x => x.IsActive && x.TaxYear != taxYear)
          .ToListAsync();

      foreach (var upload in olderActiveUploads)
