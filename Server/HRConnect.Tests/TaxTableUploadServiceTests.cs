...
using Xunit;
    ...
    public class TaxTableUploadServiceTests
    {
        ...
        [Fact]
        public async Task UploadTaxTableAsyncValidExcelCreatesUpload()
        {
            ...
            Assert.Equal(new DateTime(taxYear, 3, 1), upload.EffectiveFrom);
            Assert.Null(upload.EffectiveTo);
        }
        ...
        [Fact]
        public async Task UploadTaxTableAsyncNewUploadSetsPreviousExpiry()
        {
            using var context = GetDbContext();
            ...
            await service.UploadTaxTableAsync(2026, file);

            var uploads = await context.TaxTableUploads.ToListAsync();

            var previous = uploads.First(x => x.TaxYear == 2025);
            var latest = uploads.First(x => x.TaxYear == 2026);

            // Previous upload should have EffectiveTo set to 28 Feb 2026
            Assert.Equal(new DateTime(2026, 2, 28), previous.EffectiveTo);

            // New upload EffectiveTo remains null
            Assert.Null(latest.EffectiveTo);
            Assert.Equal(new DateTime(2026, 3, 1), latest.EffectiveFrom);
        }
        ...
        public async Task UploadTaxTableAsyncSameYearThrowsException()
        {
            ...
            // Existing upload for the same year
            context.TaxTableUploads.Add(new TaxTableUpload
            {
                TaxYear = 2026,
                FileName = "old.xlsx",
